...
export function validateRepositoryMetadata(repoRoot, mode) {
  ...
  if (mode === "release") {
    ...
    if (metadata.repositoryUrl?.match(/github\.com\/[^/]+\/cited(?:\.git)?$/)) {
      const org = metadata.organization ?? "";
      const repo = metadata.repositoryName ?? "";
      if (`${org}/${repo}` === "accomplish999/cited") {
        violations.push({
...
export function validateGitState(repoRoot, mode) {
  ...
  if (status.tags.includes(TARGET_TAG)) {
    violations.push({
...
export function generateReleaseArtifacts(repoRoot, outputDir) {
  ...
  const licenseInventoryPath = writeLicenseInventory(repoRoot, outputDir);
