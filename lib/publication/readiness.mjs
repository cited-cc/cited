  ...
  if (fileExists(packageJsonPath)) {
    ...
    for (const scriptName of requiredScripts) {
      if (!pkg.scripts?.[scriptName]) {
        findings.push({
          ruleId: "documentation-script-missing",
          category: "documentation-readiness",
          severity: "blocking",
          path: "package.json",
          message: `npm script "${scriptName}" is required for Phase 15 documentation checks.`,
        });
      }
    }
  }
...
const SECURITY_BASELINE_FILES = [
  "config/security-baseline.json",
  "scripts/check-security-baseline.mjs",
  "lib/security/egress.ts",
 ...
 * @returns {Finding[]}
 */
export function evaluateSecurityBaselinePresent(repoRoot) {
  /** @type {Finding[]} */
  const findings = [];

  for (const relativePath of SECURITY_BASELINE_FILES) {
    if (!fileExists(join(repoRoot, relativePath))) {
      findings.push({
        ruleId: "security-baseline-missing",
        category: "security-baseline-present",
        severity: "blocking",
        path: relativePath,
        message: "Required Phase 13 security baseline artifact is missing.",
      });
    }
  }
  ...
  try {
    ...
    findings.push({
      ruleId: "security-scripts-unreadable",
      category: "security-baseline-present",
      severity: "blocking",
      path: "package.json",
      message: "Could not verify security npm scripts.",
    });
    return findings;
  }
  ...
  for (const scriptName of [
    ...
    if (!packageJson.scripts?.[scriptName]) {
      findings.push({
        ruleId: "security-script-missing",
        category: "security-baseline-present",
        severity: "blocking",
        path: "package.json",
 ...
 * @returns {Finding[]}
 */
export function evaluateSecurityDocumentationPresent(repoRoot) {
  /** @type {Finding[]} */
  const findings = [];
  const requiredDocs = [
    "docs/security/threat-model.md",
  ...
  for (const docPath of requiredDocs) {
    if (!fileExists(join(repoRoot, docPath))) {
      findings.push({
        ruleId: "security-documentation-missing",
        category: "security-documentation-present",
        severity: "blocking",
        path: docPath,
        message: "Required security documentation is missing.",
      });
    }
  }

  const securityMd = join(repoRoot, "SECURITY.md");
  if (fileExists(securityMd)) {
    const content = readFileSync(securityMd, "utf8");
    if (!content.includes("threat-model")) {
      findings.push({
        ruleId: "security-md-threat-model-link",
        category: "security-documentation-present",
        severity: "blocking",
        path: "SECURITY.md",
        message: "SECURITY.md must link to the threat model.",
      });
    }
  }
 ...
 * @returns {Finding[]}
 */
export function evaluateCleanInstallVerification(repoRoot) {
  /** @type {Finding[]} */
  const findings = [];
  const lockfile = join(repoRoot, "package-lock.json");
  if (!fileExists(lockfile)) {
    findings.push({
      ruleId: "clean-install-lockfile-missing",
      category: "clean-install-verification",
      severity: "blocking",
      path: "package-lock.json",
      message: "package-lock.json is required for reproducible clean installs.",
    });
    return findings;
  }
  ...
  try {
    execFileSync("npm", ["ci", "--ignore-scripts"], {
      ...
      stdio: "pipe",
      encoding: "utf8",
      env: { ...process.env, npm_config_audit: "false", CI: "true" },
    });
  } catch {
    findings.push({
      ruleId: "clean-install-failed",
      category: "clean-install-verification",
      severity: "blocking",
      path: "package-lock.json",
      message: "npm ci failed during clean install verification.",
    });
  }
...
const PHASE14_CI_FILES = [
  ...
  "docs/maintainers/actions-supply-chain.md",
  ...
  "scripts/docs/check-readme.mjs",
  "scripts/docs/check-assets.mjs",
  "scripts/docs/check-links.mjs",
  "scripts/docs/screenshots.mjs",
  "scripts/db-migration-ci.mjs",
 ...
 * @returns {Finding[]}
 */
export function evaluateCiReadiness(repoRoot) {
  /** @type {Finding[]} */
  const findings = [];

  for (const relativePath of PHASE14_CI_FILES) {
    if (!fileExists(join(repoRoot, relativePath))) {
      findings.push({
        ruleId: "ci-artifact-missing",
        category: "ci-readiness",
        severity: "blocking",
        path: relativePath,
        message: "Required Phase 14 CI artifact is missing.",
      });
    }
  }
  ...
  if (fileExists(packageJsonPath)) {
    ...
    for (const scriptName of requiredScripts) {
      if (!pkg.scripts?.[scriptName]) {
        findings.push({
          ruleId: "ci-script-missing",
          category: "ci-readiness",
          severity: "blocking",
          path: "package.json",
          message: `npm script "${scriptName}" is required for Phase 14 CI.`,
        });
      }
    }
  }
...
export function evaluatePublicationReadiness(policy, repoRoot, trackedFiles) {
  ...
  if (policyRequiresCheck(policy, "security-baseline-present")) {
    findings.push(...evaluateSecurityBaselinePresent(repoRoot));
  }
  if (policyRequiresCheck(policy, "security-documentation-present")) {
    findings.push(...evaluateSecurityDocumentationPresent(repoRoot));
  }
  if (policyRequiresCheck(policy, "public-surface-verification")) {
    findings.push(...evaluatePublicSurfaceVerification(repoRoot));
  }
  if (policyRequiresCheck(policy, "clean-install-verification")) {
    findings.push(...evaluateCleanInstallVerification(repoRoot));
  }
  if (policyRequiresCheck(policy, "ci-readiness")) {
    findings.push(...evaluateCiReadiness(repoRoot));
  }
